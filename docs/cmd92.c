...
/*  libcalcprotocols - link protocol library, a part of the CalcForge project
...
int ti92_send_VAR_h(CalcHandle* handle, uint32_t varsize, uint8_t vartype, char *varname)
{
  ...
  calcconv_varname_to_utf8_s(handle->model, varname, trans, vartype);
  ...
  calcprotocols_info(" PC->TI: VAR (size=0x%08X=%i, id=%02X, name=%s)", varsize, varsize, vartype, trans);
...
{
  calcprotocols_info(" PC->TI: XDP (0x%04X = %i bytes)", length, length);
...
int ti92_send_SKP_h(CalcHandle* handle, uint8_t rej_code)
{
  ...
  calcprotocols_info(" PC->TI: SKP (rejection code = %i)", rej_code);
...
int ti92_send_KEY_h(CalcHandle* handle, uint16_t scancode)
{
	...
	calcprotocols_info(" PC->TI: KEY");
	TRYF(calccables_cable_send(handle->cable, buf, 4));
...
int ti92_send_REQ_h(CalcHandle* handle, uint32_t varsize, uint8_t vartype, char *varname)
{
  ...
  calcprotocols_info(" PC->TI: REQ (size=0x%08X=%i, id=%02X, name=%s)", varsize, varsize, vartype, varname);
...
int ti92_send_RTS_h(CalcHandle* handle, uint32_t varsize, uint8_t vartype, char *varname)
{
  ...
  calcprotocols_info(" PC->TI: RTS (size=0x%08X=%i, id=%02X, name=%s)",
...
int ti92_recv_VAR_h(CalcHandle* handle, uint32_t * varsize, uint8_t * vartype, char *varname)
{
  ...
  *vartype = buffer[4];
  ...
  calcprotocols_info(" TI->PC: VAR (size=0x%08X=%i, id=%02X, name=%s)",
	  *varsize, *varsize, *vartype, varname);
...
int ti92_recv_SKP_h(CalcHandle* handle, uint8_t * rej_code)
{
  ...
  *rej_code = 0;
  ...
  calcprotocols_info(" TI->PC: SKP (rejection code = %i)", *rej_code = buffer[0]);
...
int ti92_recv_XDP_h(CalcHandle* handle, uint32_t * length, uint8_t * data)
{
  ...
  *length = len;
  ...
  calcprotocols_info(" TI->PC: XDP (%04X=%i bytes)", *length, *length);
  TRYF(err);
